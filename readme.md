...
`load-test/` 디렉터리에 단계별 검증 시나리오를 구성했습니다.
...
Spike Test 이후 Publisher의 발행 속도와 Inventory Worker의 처리 속도, Kinesis Iterator Age를 비교합니다.

이를 통해 주문 요청 증가 이후에도 이벤트가 유실되지 않고 최종적으로 처리되는지 확인합니다.

### 5. 재고 부족 테스트

동일한 상품에 재고보다 많은 주문을 동시에 발생시킵니다.

조건부 재고 차감을 통해 다음 항목을 검증합니다.
...
Inventory Worker를 일시적으로 중단한 상태에서 주문 이벤트를 발생시킵니다.

이후 Worker를 복구하여 다음 항목을 확인합니다.

* Worker 장애 중 주문 API 정상 동작
* 이벤트가 Kinesis에 보존
* Worker 복구 후 미처리 이벤트 소비
* 최종 재고 처리 완료
* 컴포넌트 장애가 Order API로 직접 전파되지 않음

### 7. 중복 이벤트 테스트

동일한 `event_id`를 가진 이벤트를 중복으로 전달하여 Inventory Worker의 멱등성 처리를 검증합니다.

`processed_events`에서 처리 여부를 확인하므로 동일한 이벤트가 다시 전달되어도 재고는 한 번만 차감됩니다.

### 8. 최종 상태 확인

모든 테스트가 끝난 후 다음 항목을 확인합니다.
...
| 문제               | 원인                          | 개선 내용                            | 검증 결과                |
| ---------------- | --------------------------- | -------------------------------- | -------------------- |
| RDS 연결 수 증가      | 여러 애플리케이션 Pod가 동시에 DB 연결 사용 | RDS를 `db.t3.small`로 변경하고 연결 풀 조정 | 부하 테스트 및 전체 파이프라인 정상 |
| 순간 트래픽 증가        | Order API 처리량 급증            | HPA 최소 2개, 최대 4개, 목표 CPU 50% 적용  | 부하 증가 시 Pod 자동 확장    |
...
* DB Migration 자동화
* Alertmanager의 Slack 및 이메일 알림 연동
* HTTPS와 사용자 도메인 적용
